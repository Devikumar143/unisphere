...
router.post('/upload-media', upload.single('media'), async (req, res) => {
    console.log(`[Push-Debug] [Posts] Upload request received. File:`, req.file ? req.file.originalname : 'NONE');

    if (!req.file) {
        console.error(`[Push-Debug] [Posts] Upload failed: No file in request`);
